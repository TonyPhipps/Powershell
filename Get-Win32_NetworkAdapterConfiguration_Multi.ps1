####################################################################################
#.Synopsis 
#    Performs Get-WmiObject -class Win32_NetworkAdapterConfiguration | Select-Object *; on several systems.
#
#.Description 
#    Performs Get-WmiObject -class Win32_NetworkAdapterConfiguration on several systems.
#	 Output errors to Get-Win32_NetworkAdapterConfiguration_Multi_errors.txt.
#
#.Parameter InputList  
	...
	PROCESS{
		TRY{
			foreach ($thisHost in $INPUTLIST){
				Get-WmiObject -class Win32_NetworkAdapterConfiguration -ComputerName $thisHost -ErrorAction Stop | Select-Object *;
			};
		}
		CATCH{
